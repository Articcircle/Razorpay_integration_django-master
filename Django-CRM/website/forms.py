...
class AddRecordForm(forms.ModelForm):
	...
	product_id = forms.CharField(required=False, widget=forms.TextInput(attrs={"placeholder":"Product ID", "class":"form-control"}), label="")
	order_day = forms.DateField(required=False, widget=forms.DateInput(attrs={"type":"date", "class":"form-control"}), label="Order Day")
	delivered_date = forms.DateField(required=False, widget=forms.DateInput(attrs={"type":"date", "class":"form-control"}), label="Delivered Date")
	customer_response = forms.CharField(required=False, widget=forms.Textarea(attrs={"placeholder":"Customer Response", "class":"form-control", "rows":3}), label="")
	status = forms.ChoiceField(choices=Record.STATUS_CHOICES, widget=forms.Select(attrs={"class":"form-control"}), label="Status")
	rating = forms.IntegerField(required=False, min_value=0, max_value=5, widget=forms.NumberInput(attrs={"class":"form-control"}), label="Rating")
